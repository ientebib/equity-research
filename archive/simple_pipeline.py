...
class SimpleConfig:
    ...
    # Discovery options
    use_discovery: bool = True  # Use Discovery Agent for smart thread generation
    fallback_verticals: list[str] = field(default_factory=lambda: [
...
class ResearchBundle:
    ...
    # Discovery context (populated when using Discovery Agent)
    discovery_output: DiscoveryOutput | None = None
    research_threads: list[DiscoveredThread] = field(default_factory=list)
        ...
        result = {
            ...
            "verticals": {
                name: {
                    ...
                    "thread_priority": r.thread_priority,
...
class SimplePipeline:
    ...
    async def run(self, ticker: str) -> PipelineResult:
        ...
        try:
            ...
            # Stage 2: DISCOVERY (optional)
            discovery_output = None
            if self.config.use_discovery:
                self._progress("DISCOVERY", "Identifying research threads...", 0.32)
                discovery_output = await self._stage_discovery(ticker, company_context, output_dir)
                if discovery_output:
                    self._progress(
                        "DISCOVERY",
                        f"Found {len(discovery_output.research_threads)} threads",
                        0.38,
                    )
                else:
                    self._progress("DISCOVERY", "Using fallback verticals", 0.38)

            # Stage 3: RESEARCH
            self._progress("RESEARCH", "Starting deep research...", 0.40)
            research_bundle = await self._stage_research(ticker, company_context, discovery_output, output_dir)
            self._progress("RESEARCH", f"Research complete: {research_bundle.total_citations} citations", 0.75)

            # Stage 4: SYNTHESIS
            self._progress("SYNTHESIS", "Synthesizing research...", 0.78)
    ...
    async def _stage_discovery(
        ...
        """Stage 2: Discovery with Anthropic primary, OpenAI fallback.

        Uses the 7-lens framework to identify research threads:
        1. Official Structure (from 10-K)
        2. Competitive Cross-Reference
        3. Analyst Attention
        4. Recent Deals & Developments
        5. Asset Inventory
        6. Management Emphasis
        7. Blind Spots Synthesis

        Returns:
            DiscoveryOutput with research threads, or None to use fallback verticals.
        """
        # Try Anthropic first
        try:
            from er.agents.discovery_anthropic import AnthropicDiscoveryAgent

            self._progress("DISCOVERY", "Using Anthropic Discovery (primary)", 0.32)
            logger.info("[DISCOVERY] Using Anthropic (primary)")

            agent = AnthropicDiscoveryAgent(
                evidence_store=self.evidence_store,
            )

            discovery_output = await agent.run(company_context)

            # Save checkpoint
            checkpoint_path = output_dir / "stage2_discovery.json"
            checkpoint_data = {
                "ticker": ticker,
                "method": "anthropic",
                ...
                "threads": [
                    {
                        ...
                        "thread_type": t.thread_type.value if hasattr(t.thread_type, "value") else str(t.thread_type),
                        "is_official_segment": t.is_official_segment,
                        "discovery_lens": t.discovery_lens,
            ...
            logger.info(
                "[DISCOVERY] Anthropic complete",
                threads=len(discovery_output.research_threads),
                groups=len(discovery_output.research_groups),
            )

            return discovery_output

        except Exception as e:
            logger.warning(f"[DISCOVERY] Anthropic failed: {e}, trying OpenAI fallback")

        # Fallback to OpenAI
        try:
            from er.agents.discovery import DiscoveryAgent
            from er.agents.base import AgentContext
            from er.types import RunState, Phase, generate_id

            self._progress("DISCOVERY", "Using OpenAI Discovery (fallback)", 0.32)
            logger.info("[DISCOVERY] Using OpenAI (fallback)")

            # Create minimal context for OpenAI discovery
            run_state = RunState(
            ...
            # OpenAI Discovery requires AgentContext
            # This is more complex - for now, skip OpenAI fallback
            # and go directly to hardcoded verticals
            raise NotImplementedError("OpenAI Discovery requires full AgentContext setup")

        except Exception as e:
            logger.error(f"[DISCOVERY] OpenAI fallback also failed: {e}")
            logger.info("[DISCOVERY] Falling back to hardcoded verticals")
            return None  # Will use fallback_verticals
    ...
    async def _stage_research(
        ...
        """Stage 3: Deep research using Anthropic with Discovery threads.

        If discovery_output is provided, uses Discovery-generated threads.
        Otherwise, falls back to hardcoded verticals.
        """
...
"""
        ...
        # Build thread brief lookup
        brief_map: dict[str, ThreadBrief] = {}
        if discovery_output and discovery_output.thread_briefs:
            brief_map = {b.thread_id: b for b in discovery_output.thread_briefs}

        # Get threads to research
        research_threads: list[DiscoveredThread] = []
        if discovery_output and discovery_output.research_threads:
            # Use Discovery-generated threads, sorted by priority
            research_threads = sorted(discovery_output.research_threads, key=lambda t: t.priority)
            logger.info(f"Using {len(research_threads)} Discovery threads")
        else:
            # Fallback to hardcoded verticals
            research_threads = [
                DiscoveredThread.create(
                    name=v,
                    description=v,
                    thread_type=ThreadType.CROSS_CUTTING,
                    priority=i,
                    discovery_lens="fallback",
                    is_official_segment=False,
                    value_driver_hypothesis=f"Understand {v}",
                    research_questions=[f"What are the key aspects of {v}?"],
                    evidence_ids=[],
                )
                for i, v in enumerate(self.config.fallback_verticals)
            ]
            logger.info(f"Using {len(research_threads)} fallback verticals")
        ...
        for i, thread in enumerate(research_threads):
            self._progress(
                "RESEARCH",
                f"[P{thread.priority}] Researching: {thread.name}",
                0.40 + (i / len(research_threads)) * 0.3,
            )

            try:
                brief = brief_map.get(thread.thread_id)

                # Use research_thread() for Discovery threads, research_vertical() for fallback
                if discovery_output:
                    result = await self.researcher.research_thread(
                        ticker=ticker,
                        thread=thread,
                        brief=brief,
                        context=context,
                    ...
                    result = await self.researcher.research_vertical(
                        ...
                        context=context,
                        recency_days=90 if "recent" in thread.name.lower() else None,
                    )

                verticals[thread.name] = result
                all_evidence_ids.extend(result.evidence_ids)
                total_citations += len(result.citations)
                total_searches += result.search_count

                logger.info(
                    f"Thread complete: {thread.name}",
                ...
                logger.error(f"Research failed for {thread.name}: {e}")
        ...
        bundle = ResearchBundle(
            ...
            research_groups=discovery_output.research_groups if discovery_output else [],
        )

        # Save checkpoint
        checkpoint_path = output_dir / "stage3_research.json"
    ...
    async def _stage_synthesis(
        ...
        """Stage 4: Synthesize research into investment thesis with priority awareness."""

        # Sort research by thread priority (if available)
        sorted_items = sorted(
            research_bundle.verticals.items(),
            key=lambda x: x[1].thread_priority or 999
        )

        # Compile research content with priority markers
        research_content = ""
        for name, result in sorted_items:
            priority_marker = f"[P{result.thread_priority}]" if result.thread_priority else ""
            thread_type = f"({result.thread_type})" if result.thread_type else ""
            research_content += f"\n## {priority_marker} {name} {thread_type}\n\n{result.content}\n"

        # Build enhanced synthesis prompt
        priority_hint = ""
        if research_bundle.discovery_output:
            priority_hint = """
The research is organized by PRIORITY (P1 = highest priority):
- SEGMENT threads cover official business units
- OPTIONALITY threads cover strategic bets and hidden value
- CROSS_CUTTING threads cover themes across segments

Weight your synthesis by priority - P1 findings should have most influence on thesis.
"""
...
1. Executive Summary (2-3 paragraphs, weight findings by priority)
2. Investment Thesis (focus on P1-P2 threads if priority markers present)
3. Bull Case (emphasize OPTIONALITY upside if available)
4. Bear Case (emphasize SEGMENT risks if available)
...
{{
    ...
    "catalysts": ["catalyst1", "catalyst2", "catalyst3"]
        ...
        # Call Claude Opus 4.5 for synthesis (highest quality reasoning)
        ...
        # Get price from profile (quote is not a CompanyContext attribute)
        current_price = company_context.profile.get("price") if company_context.profile else None

        try:
            assumption_builder = AssumptionBuilder()
            dcf_inputs = assumption_builder.build_dcf_inputs(company_context)

            # Get net debt and shares from balance sheet
            balance = company_context.balance_sheet_annual[0] if company_context.balance_sheet_annual else {}
            total_debt = float(balance.get("totalDebt", 0) or 0)
            cash = float(balance.get("cashAndCashEquivalents", 0) or 0)
            net_debt = total_debt - cash

            shares = float(company_context.profile.get("sharesOutstanding", 1) or 1) if company_context.profile else 1.0

            dcf_engine = DCFEngine()
            dcf_result = dcf_engine.calculate_dcf(dcf_inputs, net_debt=net_debt, shares_outstanding=shares)
...
| Metric | Value |
|--------|-------|
| Current Price | {f'${synthesis.current_price:.2f}' if synthesis.current_price else 'N/A'} |
| Intrinsic Value | {f'${synthesis.intrinsic_value:.2f}' if synthesis.intrinsic_value else 'N/A'} |
| Upside/Downside | {f'{synthesis.upside_downside:.1%}' if synthesis.upside_downside else 'N/A'} |
...
async def run_simple_pipeline(
    ...
    Args:
        ...
        use_discovery: Whether to use Discovery Agent for smart thread generation.
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Run equity research pipeline",
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=True,
        help="Use Discovery Agent for smart thread generation (default)",
    )
    parser.add_argument(
        "--no-discovery",
        dest="use_discovery",
        action="store_false",
        help="Skip Discovery Agent, use hardcoded verticals",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
    ...
    print(f"Discovery: {'enabled' if args.use_discovery else 'disabled'}")
    ...
    if result.research_bundle.discovery_output:
        print(f"Discovery threads: {len(result.research_bundle.research_threads)}")
